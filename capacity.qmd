...
statewide_capacity <- regional_capacity %>%
      ...
      summarize(statewide_capacity = sum(total_capacity, na.rm = TRUE)) %>%
      arrange(statewide_capacity, .by_group = TRUE) %>%
      mutate(prime_mover = factor(prime_mover, 
                                  levels = unique(prime_mover)),
             year = as.character)
...
statewide_html <- ggplot(statewide_capacity,
       aes(x = year, y = statewide_capacity, fill = prime_mover)) + 
  geom_bar_interactive(aes(tooltip = round(statewide_capacity, 1)), stat = "identity", hover_nearest = TRUE) + 
  scale_y_continuous(limits = c(-200, 2000)) +
  #geom_vline(xintercept = c(3.5, 4.5), col = "black") +
  scale_fill_manual(
    values = rev(c("#606571", "#9da7bf", "#00a1b7", "#F79646", "#fad900","#9BBB59","#71346a","#896D09"))) +
  #geom_label(aes(label = round(statewide_capacity, 1)), fill = "white", size = 2) +
  facet_wrap(~prime_mover, ncol = 1, axes = "margins", scales = "fixed") +
  theme_minimal() +
  theme(panel.grid.minor.x = element_blank(),
        axis.text.y = element_blank()) #+
  #geom_label_interactive(aes(label = round(statewide_capacity, 1)),
  #                       tooltip = round(statewide_capacity$statewide_capacity, 1),
      #                   fill = "white",
     #                    hover_nearest = TRUE)
  
x <- girafe(code = print(statewide_html))
       girafe_options(x = x, opts_tooltip(delay_mouseover = 0,
                                          delay_mouseout = 1000))
